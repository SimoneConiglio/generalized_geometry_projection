...
# Import our new SCP framework
from scp_uno.mma import MMA
...
class GGPHybridPhysicsDiscipline(GGPPhysicsFastDiscipline):
    def __init__(self, solver, mesh, mesh_area, volfrac, emptyelts=None, L=60.0, H=30.0, bc_type="Short_Cantilever", max_iter=50):
        ...
        self.Path = f"Optimization_history_{bc_type}_SCP_MMA_UNO/"
    ...
    def _run(self, input_data=None):
        ...
        E_vals = rho_E * E0
        if self.emptyelts: rho_E[self.emptyelts] = 0.0; rho_V[self.emptyelts] = 0.0; E_vals[self.emptyelts] = float(self.solver.Emin)
        ...
        self.cvec.append(compliance); self.vvec.append(volume)
        print(f" It.:{len(self.cvec):5d} Obj.:{compliance:4.3e} Vol.:{volume:7.3f}")

def run_ggp_scp(bc_type="Short_Cantilever", max_iter=300):
    L, H = 60.0, 30.0; nelx, nely = 60, 30; volfrac = 0.4
    mesh = df.RectangleMesh.create([df.Point(0, 0), df.Point(L, H)], [nelx, nely], df.CellType.Type.quadrilateral)
    V_u = df.VectorFunctionSpace(mesh, "CG", 1); f_vec = np.zeros(V_u.dim()); bc = []
    ...
    xp = np.linspace(0.0, nelx, ncx + 2); yp = np.linspace(0.0, nely, ncy + 2)
    xx, yy = np.meshgrid(xp, yp); xx_flat, yy_flat = xx.flatten(order='F'), yy.flatten(order='F')
    Xc_init, Yc_init = np.tile(xx_flat, 2), np.tile(yy_flat, 2)
    ...
    hc_init = 2.0 * np.ones_like(Xc_init); Mc_init = 0.5 * np.ones_like(Xc_init)
    x_init = np.column_stack([Xc_init, Yc_init, Lc_init, hc_init, Tc_init, Mc_init]).flatten()
    lb = np.column_stack([(np.min(xx_flat)-1.)*np.ones_like(Xc_init), (np.min(yy_flat)-1.)*np.ones_like(Xc_init), np.zeros_like(Xc_init), np.ones_like(Xc_init), -2.*np.pi*np.ones_like(Xc_init), np.zeros_like(Xc_init)]).flatten()
    ub = np.column_stack([(np.max(xx_flat)+1.)*np.ones_like(Xc_init), (np.max(yy_flat)+1.)*np.ones_like(Xc_init), np.sqrt(nelx**2+nely**2)*np.ones_like(Xc_init), np.sqrt(nelx**2+nely**2)*np.ones_like(Xc_init), 2.*np.pi*np.ones_like(Xc_init), np.ones_like(Xc_init)]).flatten()
    ...
    geom_disc = GGPVectorizedGeometryDiscipline(mesh, len(Xc_init), "Free", L_domain=L, H_domain=H, Ngp=2, r_gp=0.5, pp=100.0, lb=lb, ub=ub)
    phys_disc.geom_disc = geom_disc; chain = MDAChain([geom_disc, phys_disc])
    
    from gemseo import create_design_space
    ds = create_design_space()
    # START FROM Grid, but unscaled to 0.4 if needed.
    ds.add_variable("x_vars", size=len(lb), lower_bound=0.0, upper_bound=1.0, value=((x_init-lb)/(ub-lb)))
    
    from gemseo.algos.optimization_problem import OptimizationProblem
    from gemseo.core.mdo_functions.mdo_function import MDOFunction
    def obj_func_call(x): chain.execute({"x_vars": x}); return chain.local_data["compliance"]
    def obj_jac_call(x): return chain.linearize({"x_vars": x}, compute_all_jacobians=True)["compliance"]["x_vars"]
    def vol_func_call(x): chain.execute({"x_vars": x}); return chain.local_data["volume"]
    def vol_jac_call(x): return chain.linearize({"x_vars": x}, compute_all_jacobians=True)["volume"]["x_vars"]
    ...
    problem = OptimizationProblem(ds); problem.objective = obj_func; problem.add_constraint(vol_func, constraint_type="ineq")
    problem.lagrange_multipliers_computation = False 

    # ROBUST SETTINGS TO REACH 75
    mma_algo = MMA()
    options = {
        "max_iter": max_iter,
        "inner_library": "scipy",
        "inner_solver": "SLSQP", 
        "max_optimization_step": 0.02, # Very stable steps
        "initial_asymptotes_distance": 0.05,
        "ftol_rel": 1e-12, "xtol_rel": 1e-12, "ftol_abs": 1e-12, "xtol_abs": 1e-12,
        "kkt_tol_abs": -1.0, "kkt_tol_rel": -1.0
    }
    ...
    sc.kkt_residual_computation = dummy_kkt; bol.kkt_residual_computation = dummy_kkt

    print("\nStarting SCP-MMA Optimization with Stable Settings...")
    mma_algo.execute(problem, **options)
    if len(phys_disc.cvec) > 0: print(f"\nFinal Compliance: {phys_disc.cvec[-1]:.4f}, Volume: {phys_disc.vvec[-1]*100.0:.4f}%")

if __name__ == "__main__":
    run_ggp_scp(max_iter=300)
