...
"""GEMSEO integration tests for the ``MLS_SBO`` algorithm (skipped without GEMSEO)."""
import numpy as np
import pytest
...
from scp_uno.mls_sbo import MLSSurrogateSBO
...
def test_algorithm_is_described():
    infos = MLSSurrogateSBO.ALGORITHM_INFOS["MLS_SBO"]
    assert infos.require_gradient
    assert infos.handle_inequality_constraints
    assert infos.Settings is MLSSBOSettings


def test_mls_sbo_solves_gemseo_problem():
    problem = _quadratic_problem(d=5)
    library = MLSSurrogateSBO("MLS_SBO")
