...
def run_l_shape_bracket(max_iter=50, validate=False):
    L, H = 100.0, 100.0
    nelx, nely = 100, 100
    volfrac = 0.4
    num_components = 18
    ...
    class MiddleRightArea(df.SubDomain):
        def inside(self, x, on_boundary): return df.near(x[0], L, 1.0) and df.near(x[1], H/2.0, 5.0)
    ...
    # Validator
    validator = StepByStepValidator(output_dir="validation_l_shape") if validate else None
