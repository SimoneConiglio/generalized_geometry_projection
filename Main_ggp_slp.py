"""Main script for SLP (and line-search variants) on the Short Cantilever benchmark.

Usage
-----
    # Pure SLP
    python Main_ggp_slp.py --algo slp --max_iter 300

    # SLP + monotone line-search
    python Main_ggp_slp.py --algo slp_ls --max_iter 300

    # CONLIN + monotone line-search (for comparison)
    python Main_ggp_slp.py --algo conlin_ls --max_iter 300
"""
...
# SCP solvers
from scp_uno.slp import SLP
from scp_uno.conlin import CONLIN


# ---------------------------------------------------------------------------
# Physics discipline with per-iteration plotting
# ---------------------------------------------------------------------------
...
class GGPHybridPhysicsDiscipline(GGPPhysicsFastDiscipline):
    def __init__(
        ...
        self.Path = f"Optimization_history_{bc_type}_SCP_{run_label}/"
    ...
    def _run(self, input_data=None):
        ...
        self.local_data["volume"] = np.array(
            [(volume - self.volfrac) / self.volfrac * 100.0]
        )

        grad_C = np.zeros(self.num_elements)
        for i in range(self.num_elements):
            grad_C[i] = -E0 * np.dot(
                u[self.cell_dofs[i]], np.dot(self.ke_ref, u[self.cell_dofs[i]])
            )
        self.dj_drhoE = grad_C / (compliance + 1.0)
        self.dv_drhoV = np.ones(self.num_elements) * (
            100.0 / (self.volfrac * self.num_elements)
        )
        if self.emptyelts:
    ...
    def _save_plots(self, outit, compliance, volume, rho_V):
        ...
        if self.geom_disc is not None:
            ...
            cols = np.round(
                (self.geom_disc.X_mesh - dx / 2.0) / dx
            ).astype(int)
            rows_idx = np.round(
                (self.geom_disc.Y_mesh - dy / 2.0) / dy
            ).astype(int)
            ...
            if self.geom_disc.lb is not None and self.geom_disc.ub is not None:
                x_vars = (
                    self.geom_disc.lb + x_vars * (self.geom_disc.ub - self.geom_disc.lb)
                )
...
# ---------------------------------------------------------------------------
# Problem setup (mesh, BCs, geometry initialisation) — shared across algos
# ---------------------------------------------------------------------------

def _build_problem(max_iter, run_label):
    L, H = 60.0, 30.0
    nelx, nely = 60, 30
    volfrac = 0.4

    mesh = df.RectangleMesh.create(
        [df.Point(0, 0), df.Point(L, H)],
        [nelx, nely],
    ...
    tip = np.intersect1d(
        np.where(np.linalg.norm(dof_coords - np.array([L, H / 2.0]), axis=1) < 1e-3)[0],
        y_dofs,
    )[0]
    f_vec[tip] = -1.0
    ...
    Lc_init = (
        2.0
        * np.sqrt((nelx / (ncx + 2)) ** 2 + (nely / (ncy + 2)) ** 2)
        * np.ones_like(Xc_init)
    )
    angle = np.arctan2(nely / ncy, nelx / ncx)
    Tc_init = angle * np.concatenate(
        [np.ones(len(Xc_init) // 2), -np.ones(len(Xc_init) // 2)]
    )
    hc_init = 2.0 * np.ones_like(Xc_init)
    Mc_init = 0.5 * np.ones_like(Xc_init)
    x_init = np.column_stack(
        [Xc_init, Yc_init, Lc_init, hc_init, Tc_init, Mc_init]
    ).flatten()
    lb = np.column_stack([
    ...
    phys_disc = GGPHybridPhysicsDiscipline(
        solver, mesh, L * H, volfrac, None, L, H,
        "Short_Cantilever", max_iter, plot_rate=10, run_label=run_label,
    )
    phys_disc.f_vec = f_vec
    phys_disc.f_vec[phys_disc.fixed_dofs] = 0.0
    geom_disc = GGPVectorizedGeometryDiscipline(
        mesh, len(Xc_init), "Free",
        L_domain=L, H_domain=H, Ngp=2, r_gp=0.5, pp=100.0, lb=lb, ub=ub,
    )
    ...
    def obj_jac_call(x):
        return chain.linearize(
            {"x_vars": x}, compute_all_jacobians=True
        )["compliance"]["x_vars"]
    ...
    # Silence KKT residual (not meaningful for our outer SCP loop)
...
ALGO_MAP = {
    "slp": (SLP, False),
    "slp_ls": (SLP, True),
    "conlin_ls": (CONLIN, True),
}

COMMON_OPTIONS = {
    "inner_library": "scipy",
    "inner_solver": "SLSQP",
    "max_optimization_step": 0.02,
    "ftol_rel": 1e-12,
    "xtol_rel": 1e-12,
    "ftol_abs": 1e-12,
    "xtol_abs": 1e-12,
...
LINE_SEARCH_OPTIONS = {
    ...
    "line_search_penalty": 500.0,  # μ — high enough to outweigh compliance spikes
    "line_search_c1": 1e-4,
...
def run(algo_key: str = "slp", max_iter: int = 300) -> None:
    if algo_key not in ALGO_MAP:
        raise ValueError(f"Unknown algo '{algo_key}'. Choose from: {list(ALGO_MAP)}")

    AlgoClass, use_ls = ALGO_MAP[algo_key]
    run_label = algo_key.upper()

    print(f"\n{'=' * 60}")
    print(f"  GGP Short Cantilever — {run_label}  (max_iter={max_iter})")
    print(f"{'=' * 60}\n")

    problem, phys_disc = _build_problem(max_iter, run_label)

    options = {**COMMON_OPTIONS, "max_iter": max_iter}
    if use_ls:
        options.update(LINE_SEARCH_OPTIONS)
    ...
    if phys_disc.cvec:
        print(
            f"\nFinal Compliance: {phys_disc.cvec[-1]:.4f}, "
            f"Volume: {phys_disc.vvec[-1] * 100:.4f}%"
        )
        print(f"Results saved to: {phys_disc.Path}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="GGP Short Cantilever — SLP / line-search variants"
    )
    parser.add_argument(
        "--algo",
        choices=list(ALGO_MAP),
        default="slp",
        help=(
            "slp: pure SLP | "
            "slp_ls: SLP + monotone line-search | "
            "conlin_ls: CONLIN + monotone line-search"
        ),
    )
    parser.add_argument(
        "--max_iter", type=int, default=300,
        help="Maximum number of outer SCP iterations",
    )
    args = parser.parse_args()
    run(algo_key=args.algo, max_iter=args.max_iter)
